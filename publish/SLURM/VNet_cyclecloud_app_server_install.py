...
def install_mariaDB():
    print("Installing MariaDB on cycle VM")
    #Install mariadb mariadb-server
    # sudo apt install -y mariadb-server
    _catch_sys_error(["apt", "install", "-y", "mariadb-server"])

    #  /etc/mysql/mariadb.conf.d/50-server.cnf
    # sed -i 's/bind-address/#bind-address/g' /etc/mysql/mariadb.conf.d/50-server.cnf
    # _catch_sys_error([" /usr/bin/mariadb <  /var/lib/waagent/custom-script/download/0/database.sql"])
    with open('/etc/mysql/mariadb.conf.d/50-server.cnf') as inputfile:
        lines = inputfile.read()
    lines = lines.replace('bind-address', '#bind-address')
    with open("/etc/mysql/mariadb.conf.d/50-server.cnf", "w") as outputfile:
        outputfile.write(lines)


    # sudo systemctl enable mariadb.service
    _catch_sys_error(["systemctl", "enable", "mariadb.service"])
    # sudo systemctl start mariadb.service
    _catch_sys_error(["systemctl", "start", "mariadb.service"])

    # echo "create database slurm_acct_db;" > database.sql
    # echo "create user 'Slurm'@'localhost'; " >> database.sql
    # echo "set password for 'Slurm'@'localhost' = password('SlurmPassword123'); " >> database.sql
    # echo "grant usage on *.* to 'Slurm'@'localhost'; " >> database.sql
    # echo "grant all privileges on slurm_acct_db.* to 'Slurm'@'localhost'; " >> database.sql
    # echo "flush privileges; " >> database.sql
    # echo "exit " >> database.sql
    # write strign to file
    sqlStr =  "create database slurm_acct_db; create user 'Slurm'@'%'; set password for 'Slurm'@'%' = password('SlurmPassword123'); grant usage on *.* to 'Slurm'@'%'; grant all privileges on slurm_acct_db.* to 'Slurm'@'%'; flush privileges; "
    # write string to file

    with open("/var/lib/waagent/custom-script/download/0/database.sql", "w") as text_file:
        text_file.write(sqlStr)
    # _catch_sys_error([" /usr/bin/mariadb <  /var/lib/waagent/custom-script/download/0/database.sql"])

    p = subprocess.run(['/usr/bin/mariadb'], input=sqlStr, capture_output=True, text=True)
    print(p.stdout)
    print(p.stderr)
    # sudo mariadb < database.sql
    _catch_sys_error(["systemctl", "restart", "mariadb.service"])

def import_cluster(vm_metadata, cluster_image, cpu_machine_type, cpu_node_size, std_cpu_node_cores, spot_cpu_node_cores, gpu_machine_type, gpu_node_size, std_gpu_node_cores, spot_gpu_node_cores, slurm_version, subnet_name, cluster_files_download_url, key, lustreMGSIpAddress, installNextflow):
    ...
    #cluster_files_download_url = "https://produssolutionrepos.blob.core.windows.net/lincolncollection-prerelease/SimpleSLURMonVNet/"
    # "https://bizdataresearch.blob.core.windows.net/public/Lincoln/"
    ...
    cluster_template_file_url = cluster_files_download_url + cluster_template_file_name + "?" + key
    cluster_parameters_file_url = cluster_files_download_url + cluster_parameters_file_name + "?" + key
    ...
    resource_group = "rg-hubnetwork-hpc-uks-01"
    vnet_name ="vnet-hub-hpc-uks-01"
    ...
    # *IMPORTANT: this string value has a dependency on the name specified to the subnet in the ARM template that deploys the CycleCloud App
    # *********************************************************
    subnet_string_value = resource_group + "/" + vnet_name + "/" + subnet_name
    subnet_param = "SubnetId=" + subnet_string_value
    print("The subnet for the worker nodes is: %s" % subnet_param)

    ##### HARD-CODING THIS FOR THIS SCENARIO SO THAT MASTER NODE HAS GUI THAT COMES WITH DSVM IMAGE #####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    schedulerImage_param = "SchedulerImageName=microsoft-dsvm:ubuntu-2004:2004-gen2:latest"
    print("The os image for the scheduler nodes is: %s" % schedulerImage_param)
    ###############################################################################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    ##### DOES NOT TAKE EFFECT BECAUSE PARAMETERS .JSON DECLARES A CUSTOM IMAGE FROM THE COMPUTE GALLERY #####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    workerImage_param = "HPCImageName=" + cluster_image
    print("The os image for the worker nodes is: %s" % workerImage_param)
    ###############################################################################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    cpu_machineType_param = "HPCMachineType=" + cpu_machine_type
    #machineType_param = "HPCMachineType=NC24ads_A100_v4"
    print("The machine type for the CPU worker nodes is: %s" % cpu_machineType_param)

    std_cpu_max_core = int(cpu_node_size) * int(std_cpu_node_cores)
    std_cpu_maxCore_param = "MaxStandardHPCExecuteCoreCount=" + str(std_cpu_max_core)
    print("The amount of execute core for the Standard CPU worker nodes is: %s" % std_cpu_maxCore_param)

    spot_cpu_max_core = int(cpu_node_size) * int(spot_cpu_node_cores)
    spot_cpu_maxCore_param = "MaxSpotHPCExecuteCoreCount=" + str(spot_cpu_max_core)
    print("The amount of execute core for the Spot CPU worker nodes is: %s" % spot_cpu_maxCore_param)

    gpu_machineType_param = "GPUMachineType=" + gpu_machine_type
    #machineType_param = "HPCMachineType=NC24ads_A100_v4"
    print("The machine type for the GPU worker nodes is: %s" % gpu_machineType_param)

    std_gpu_max_core = int(gpu_node_size) * int(std_gpu_node_cores)
    std_gpu_maxCore_param = "MaxStandardGPUExecuteCoreCount=" + str(std_gpu_max_core)
    print("The amount of execute core for the Standard GPU worker nodes is: %s" % std_gpu_maxCore_param)

    spot_gpu_max_core = int(gpu_node_size) * int(spot_gpu_node_cores)
    spot_gpu_maxCore_param = "MaxSpotGPUExecuteCoreCount=" + str(spot_gpu_max_core)
    print("The amount of execute core for the Spot GPU worker nodes is: %s" % spot_gpu_maxCore_param)
    ...
    # configuration_slurm_accounting_url
    # "network": {
    # "interface": [{
    #     "ipv4": {
    #        "ipAddress": [{
    #             "privateIpAddress":
    configuration_slurm_accounting_url_param = "configuration_slurm_accounting_url=" + vm_metadata["network"]["interface"][0]["ipv4"]["ipAddress"][0]["privateIpAddress"]
    print("The SLURM accounting IP is: %s" % configuration_slurm_accounting_url_param)
    
    # We import the cluster, passing the subnet name as a parameter override
    _catch_sys_error(["/usr/local/bin/cyclecloud","import_cluster","-f", cluster_template_file_download_path, "-p", cluster_parameters_file_download_path, "--parameter-override", location_param , "--parameter-override", subnet_param, "--parameter-override", schedulerImage_param, "--parameter-override", workerImage_param, "--parameter-override", cpu_machineType_param, "--parameter-override", std_cpu_maxCore_param,"--parameter-override", spot_cpu_maxCore_param,"--parameter-override", gpu_machineType_param, "--parameter-override", std_gpu_maxCore_param, "--parameter-override", spot_gpu_maxCore_param, "--parameter-override", slurmVersion_param , "--parameter-override" ,configuration_slurm_accounting_url_param])
...
def main():
    ...
    parser.add_argument("--sizeOfCPUWorkerNodes",
                        dest="sizeOfCPUWorkerNodes",
                        default="Standard_B2ms",
                        help="The VM size for CPU worker nodes")

    parser.add_argument("--numberOfStandardCPUWorkerNodes",
                        dest="numberOfStandardCPUWorkerNodes",
                        default=2,
                        help="The number of Standard CPU worker nodes")

    parser.add_argument("--numberOfSpotCPUWorkerNodes",
                            dest="numberOfSpotCPUWorkerNodes",
                            default=2,
                            help="The number of Spot CPU worker nodes")

    parser.add_argument("--countOfCPUNodeCores",
                        dest="countOfCPUNodeCores",
                        default=2,
                        help="The amount of cores for CPU worker nodes")

    parser.add_argument("--sizeOfGPUWorkerNodes",
                        dest="sizeOfGPUWorkerNodes",
                        default="Standard_B2ms",
                        help="The VM size for GPU worker nodes")

    parser.add_argument("--numberOfStandardGPUWorkerNodes",
                        dest="numberOfStandardGPUWorkerNodes",
                        default=2,
                        help="The number of Standard GPU worker nodes")

    parser.add_argument("--numberOfSpotGPUWorkerNodes",
                        dest="numberOfSpotGPUWorkerNodes",
                        default=2,
                        help="The number of Spot GPU worker nodes")

    parser.add_argument("--countOfGPUNodeCores",
                        dest="countOfGPUNodeCores",
                        default=2,
                        help="The amount of cores for GPU worker nodes")
    ...
    parser.add_argument("--installMariaDB",
                        dest="installMariaDB",
                        action="store_true",
                        help="Install Mariadb on CycleCloud VM")
    
    parser.add_argument("--subnetName",
                        dest="subnetName",
                        default="",
                        help="The Subnet name of the resource.")
    
    parser.add_argument("--installScriptURLPath",
                        dest="urlPath",
                        default="",
                        help="The URL path to download the slurm template from.")
    
    parser.add_argument("--installScriptURLSASKey",
                        dest="urlKey",
                        default="",
                        help="The URL SAS Key to access the slurm template from the URL Path.")
    ...
    if args.installMariaDB:
        print_timestamp()
        print("SCRIPT: Calling function to install MariaDB...")
        install_mariaDB()
    ...
    import_cluster(vm_metadata, args.osOfClusterNodes, args.sizeOfCPUWorkerNodes, args.countOfCPUNodeCores, args.numberOfStandardCPUWorkerNodes, args.numberOfSpotCPUWorkerNodes, args.sizeOfGPUWorkerNodes, args.countOfGPUNodeCores, args.numberOfStandardGPUWorkerNodes, args.numberOfSpotGPUWorkerNodes, args.slurmVersion, args.subnetName, args.urlPath, args.urlKey, lustre_mgs_ip_address, install_nextflow)
